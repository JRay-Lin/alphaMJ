...
class Stage1Trainer:
    ...
    def __init__(self, config: dict = None):
        ...
        # Create multi-agent system - use individual agents for better interpretability
        self.multi_agent = MultiAgentDQN(
            num_agents=4,
            centralized_training=False,  # Use separate agents
        ...
        self.env = MultiAgentTrainingEnv(
            rule_name=self.config["rule_name"], reward_system="stage1"
        )
    ...
    def _get_default_config(self) -> dict:
        ...
        return {
            ...
            "rule_name": "standard",
            "centralized_training": False,  # Use individual agents by default
    ...
    def _process_episode(self, episode_result: dict, episode_num: int):
        ...
        if episode_num % 50 == 0:  # Print every 50th episode
            print(
                f"Episode {episode_num}: Winner={winner}, Length={episode_length}, Reward={episode_reward:.3f}"
            )
        ...
        # Save statistics periodically to prevent memory issues
        if episode_num % 1000 == 0 and episode_num > 0:
            self._save_statistics_checkpoint(episode_num)
        ...
        if winner is not None and winner >= 0:
            ...
            recent_wins = sum(
                1 for _ in range(recent_episodes) if episode_num >= recent_episodes - 1
            )
    ...
    def _log_progress(self, episode: int):
        ...
        avg_loss = np.mean(
            [stat["average_loss"] for stat in agent_stats["agent_stats"]]
        )
    ...
    def _evaluate(self) -> dict:
        ...
        eval_results = {
            "win_rates": win_rates,
            "balance_score": 1.0
            - np.std(list(win_rates.values())),  # Higher is more balanced
            "total_games": 100,
    ...
    def _log_evaluation(self, episode: int, eval_results: dict):
        ...
        print(
            f"Agent performances: {[f'{perf:.3f}' for perf in eval_results['agent_performances']]}"
        )
        print("-" * 40)
    ...
    def _save_checkpoint(self, episode: int):
        ...
        stats_path = os.path.join(
            self.config["log_dir"], f"training_stats_{episode}.json"
        )
    ...
    def _save_statistics_checkpoint(self, episode: int):
        """Save detailed statistics checkpoint."""
        stats = {
            "episode": episode,
        ...
        stats_path = os.path.join(self.config["log_dir"], f"detailed_stats_{episode}.json")
        with open(stats_path, "w") as f:
            json.dump(stats, f)
    ...
    def _save_best_model(self, episode: int):
        ...
        metadata = {
            ...
            "model_type": (
                "shared" if self.multi_agent.centralized_training else "individual"
            ),
        }

        metadata_path = os.path.join(
            self.config["model_dir"], "best_model_metadata.json"
        )
...
def main():
    ...
    parser.add_argument(
        "--episodes", type=int, default=1000000, help="Number of training episodes"
    )
    parser.add_argument(
        "--rule",
        choices=["standard", "taiwan"],
        default="standard",
        help="Mahjong rule variant",
    )
    parser.add_argument(
        "--model-dir", type=str, default="ai/models/stage1", help="Model save directory"
    )
    parser.add_argument(
        "--log-dir", type=str, default="ai/logs/stage1", help="Log directory"
    )
    parser.add_argument(
        "--centralized",
        action="store_true",
    ...
    parser.add_argument(
        "--device", type=str, default="auto", help="Training device (cuda/cpu/auto)"
    )
    ...
    centralized_training = (
        not args.individual_agents
    )  # Default to individual unless CTDE requested
    ...
    config = {
        ...
        "centralized_training": False,  # Always use individual agents
